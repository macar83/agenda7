...
const TaskManagerApp = () => {
  ...
  // 🔔 Effetto per avviare controllo notifiche quando autenticato
  useEffect(() => {
    if (appData.data.isAuthenticated && notifications.isEnabled && appData.data.lists) {
      console.log('🔔 Avvio controllo notifiche task');
      notifications.startPeriodicCheck(appData.data.lists, 1); // Controlla ogni minuto
  ...
  const handleRequestNotifications = async () => {
    ...
    if (granted) {
      ...
      if (appData.data.lists && appData.data.lists.length > 0) {
        notifications.startPeriodicCheck(appData.data.lists, 1);
      }
    }
  };
  ...
  const renderContent = () => {
    console.log('🔍 App Debug - Current view:', appData.data.currentView);
    ...
    switch (appData.data.currentView) {
      ...
      case 'stats':
        return <StatsView />;
      case 'notifications': // 🔔 NUOVO: View notifiche
        return (
            ...
            {process.env.NODE_ENV === 'development' && (
              <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                ...
                <div className="text-xs text-yellow-700 space-y-1">
                  ...
                  <p>Liste caricate: {appData.data.lists?.length || 0}</p>
                </div>
              </div>
            )}
          </div>
      ...
      default:
        console.log('🔍 Unknown view, fallback to overview:', appData.data.currentView);
        return <Overview />;
    }
  };
    ...
    <AppContext.Provider value={appData}>
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        {appData.data.isAuthenticated && 
         ...
         !notifications.isEnabled && 
         appData.data.currentView !== 'notifications' && (
          <div className="bg-blue-50 border-b border-blue-200 px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => appData.updateData({ currentView: 'notifications' })}
